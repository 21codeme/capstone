...
class FirebaseAuthService {
  ...
  Future<UserCredential?> registerUser({
    ...
    try {
      ...
      print('🚀 [REGISTRATION] Starting user registration for: $normalizedEmail');
  ...
  Future<void> signOut() async {
    try {
      ...
      // Update user's online status in Firestore BEFORE signing out (non-blocking for faster logout)
      if (uid != null) {
        // Firestore update - don't wait for it, just fire and forget
  ...
  Future<void> _clearSharedPreferences() async {
    try {
      ...
      // Clear all auth-related preferences in parallel for faster execution
      await Future.wait([
  ...
  Future<Map<String, dynamic>?> getUserData(String uid) async {
    try {
      ...
      if (doc.exists) {
        final data = doc.data();
        print('✅ User data retrieved successfully by UID: ${data?['email']}');
  ...
  Future<bool> getUserIsStudent(String uid) async {
    try {
      print('🔍 Getting user isStudent status for UID: $uid');
      ...
      if (doc.exists) {
        ...
        if (role != null && role.isNotEmpty) {
          ...
          print('✅ Converted legacy role ($role) to isStudent: $isStudent');
  ...
  Future<bool> userExists(String email) async {
    try {
      ...
      final normalizedEmail = email.toLowerCase().trim();
      print('🔍 Checking if user exists for email: $normalizedEmail');
      ...
      try {
        ...
        print(exists 
          ? '✅ User exists in Firebase Auth (wrong-password error)' 
  ...
  Future<bool> updateAccountStatus(String uid, bool isActive) async {
    try {
      print('🔄 [ACCOUNT] Updating account status for UID: $uid to ${isActive ? 'active' : 'inactive'}');
  ...
  Future<List<Map<String, dynamic>>> getUsersByRole(String role) async {
    try {
      print('🔍 Getting users with role: $role');
  ...
  Future<bool> updateUserDocument(String uid, Map<String, dynamic> data) async {
    try {
      print('📝 Updating user document for UID: $uid');
      await _firestore.collection('users').doc(uid).update(data);
  ...
  Future<List<String>> fetchSignInMethods(String email) async {
    try {
      print('🔍 Fetching sign-in methods for email: $email');
  ...
  Future<bool> updateUserDisplayName({required User user, required String displayName}) async {
    try {
      print('👤 Updating user display name for UID: ${user.uid}');
  ...
  Future<String?> getStudentIdByEmail(String email) async {
    try {
      print('🔍 Getting student ID for email: $email');
